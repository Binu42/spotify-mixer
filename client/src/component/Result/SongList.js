...
import { Avatar, Chip, Divider, List, ListItem, ListItemAvatar, ListItemSecondaryAction, ListItemText, Tooltip, Zoom } from '@material-ui/core'
...
const SongList = ({ loading, songs }) => {
  ...
  return (
    <List component="ul">
      {
        songs?.map(song => {
          const { id, preview_url, name, album: { images }, artists, duration_ms, explicit } = song;
          ...
          return (
            <div key={id}>
              <Tooltip title={preview_url !== null ? "click to play" : "preview of this song not available"} placement="top" TransitionComponent={Zoom} >
                <ListItem onClick={e => preview_url !== null ? setPlayIndex(index.findIndex(el => el === id)) : ""} className={(currentlyPlayingId === id ? "active" : '') + (preview_url !== null ? ' pointer' : '')}>
                  <ListItemAvatar>
                    <Avatar className="track-avatar" variant="square" alt={name} src={images[0]?.url} style={{ width: 50, height: 50, borderRadius: 5, marginRight: 10 }} />
                  </ListItemAvatar>
